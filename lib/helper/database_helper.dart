...
class DatabaseHelper {
  ...
  Future<Database> _initDatabase() async {
    ...
    return openDatabase(join(path, _databaseName),
      ...
      await db.execute(
          "CREATE TABLE STUDENT(id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, email TEXT NOT NULL, dob TEXT NOT NULL, course TEXT NOT NULL, adm_no TEXT NOT NULL, mob TEXT NOT NULL, password TEXT NOT NULL)");
  ...
  Future<void> update(ModelClass modelClass, int id) async {
    final Database db = await _initDatabase();
    await db
        .update("STUDENT", modelClass.toMap(), where: "id=?", whereArgs: [id]);
  }
  ...
  Future<int> saveUser(ModelClass modelClass) async {
    final Database db = await _initDatabase();
    int res = await db.insert("STUDENT", modelClass.toMap());
    return res;
  }
}
